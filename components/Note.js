...
const Note = () => {
  ...
  const deleteBloc = (blocPosition) => {
    ...
    const updatedBlocs = noteData.blocs.filter(
      (data) => data.position != blocPosition
    );
  ...
  const setBlocsValue = (blocPosition, value) => {
    const updatedBlocs = noteData.blocs.map((data) =>
      data.position == blocPosition ? { ...data, value } : data
    );
    setNoteData((prevData) => ({
      ...prevData,
      blocs: updatedBlocs,
  ...
  const setBlocsType = (blocPosition, type) => {
    const updatedBlocs = noteData.blocs.map((data) =>
      data.position == blocPosition ? { ...data, type } : data
    );
    setNoteData((prevData) => ({
      ...prevData,
      blocs: updatedBlocs,
